...
namespace cursed.ViewModel.Tests
{
    ...
    {
        /// <summary>
        /// Проверяет, что метод UploadImage() выбрасывает FileNotFoundException при попытке загрузить изображение по неверному пути.
        /// </summary>
        [TestMethod()]
        ...
        /// <summary>
        /// Проверяет, что метод UploadImage() возвращает корректный URL изображения при загрузке изображения по валидному пути.
        /// </summary>
        [TestMethod()]
        public async Task UploadImage_ValidImagePath_ReturnsImageUrl()
        {
            var imageUploader = new FilesVM();
            var imagePath = @"../../Resources/Images/placeholder.jpg";
